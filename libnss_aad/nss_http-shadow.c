...
_nss_aad_setspent_locked(int stayopen)
{
    ...
    printf("setspent_locked\n");

    snprintf(url, 512, "http://" NSS_HTTP_SERVER ":" NSS_HTTP_PORT "/shadow");

    char *response = nss_http_request(url, NULL);
    if (!response) {
